...
*Title/Purpose 	: This do.file was developed by the Evans School Policy Analysis & Research Group (EPAR) 
				  ...
				  using the Uganda Financial Inclusion Insights Wave 1 data set.
...
*-----------
*This Master do.file constructs selected sociodemogaphic and mobile money uptake outcome variables from the Uganda Financial Inclusion Insights Wave 1 data set.
*After downloading the "Final_Uganda-FII-wave 1(public).xlsx" data file from Intermedia, we convert it to Stata .dta format for analysis. 
...
import excel using "filepath for location where you downloaded raw data/Final_Uganda-FII-wave 1(public).xlsx", firstrow
save "filepath for location where you want to save dta file/Uganda_Wave1_Stata.dta", replace
clear
...
*UGANDA*
*******
use "$input\Uganda_Wave1_Stata.dta"
...
la var highest_ed "highest level of education (DG4)"
	...
	*/ 4 "Some secondary education" 5 "Secondary education complete" 6 "Some secondary education vocational" /*
	*/ 7 "Some secondary vocational training complete" 8 "Some diploma" 9 "Diploma complete" 10 "Some college/university" /*
...
replace dfs_aware_spont=1 if (MM1A==1 | MM1B==1 | MM1C==1 | MM1D==1 | MM1E==1 | MM1F==1 | MM1G==1)
replace dfs_aware_spont=0 if (MM1A==0 & MM1B==0 & MM1C==0 & MM1D==0 & MM1E==0 & MM1F==0 & MM1G==0)
la var dfs_aware_spont "person could spontaneously recall name of any DFS provider (MM1A - MM1G)"

**PROMPTED RECALL of DFS providers
gen dfs_aware_prompt=.
replace dfs_aware_prompt=1 if MM2A==1 | MM2B==1 | MM2C==1 | MM2D==1 | MM2E==1 | MM2F==1 | MM2G==1
replace dfs_aware_prompt=0 if MM2A==0 & MM2B==0 & MM2C==0 & MM2D==0 & MM2E==0 & MM2F==0 & MM2G==0
la var dfs_aware_prompt "person could recall name of any DFS provider when prompted with the provider's name(MM2A - MM2G)"
...
*ADOPTION*
**********
gen dfs_adopt=. //MM6H indicates "none or never" 
replace dfs_adopt=1 if (MM6A==1 | MM6B==1 | MM6C==1 | MM6D==1 | MM6E==1 | MM6F==1 | MM6G==1)
replace dfs_adopt=0 if MM6H==1
la var dfs_adopt "person has ever used DFS for any financial activity (MM6A-G)"
...
la var dfs_lastuse_more90 "person used DFS over 90 days ago (MM7_1-7)"
...
save "$output\Uganda_W1_Clean.dta", replace
